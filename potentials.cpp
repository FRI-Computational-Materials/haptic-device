...
///////////////////////////// LJ ///////////////////////////////////////
// Mixed lj potential for multiatomic nanoparticle LJ //////////////
vector<vector<double>> ljCalculator::getFandU(vector<Atom*>& spheres) {
  ...
  for (int i = 0; i < spheres.size(); i++) {
	...
	double sigma, epsilon;
        tie(epsilon, sigma) = getEpsandSig(spheres[i], spheres[j]);

	// The functions were one line long and only showed up here, so I moved them
        potentialEnergy += 4 * epsilon * (pow(sigma / distance, 12) - pow(sigma / distance, 6));
        double appliedForce = -4 * epsilon * ((-12 * pow(sigma / distance, 13)) - (-6 * pow(sigma / distance, 7)));
        force.add(appliedForce * dir01);
      }
    }
  vector<double> pushBack = {force.x(), force.y(), force.z()};
...
// Finds sigma and epsilon values of two atoms
tuple<double, double> ljCalculator::getEpsandSig(Atom* a1, Atom* a2) {

  // Place elements in alphabetical order before searching
  string elem1 = a1->getElement();
  string elem2 = a2->getElement();
  string elements;
  ...
  // If found in unordered map, return tuple of epsilon and sigma, else return 1.0, 1.0
  if (epsandsig.find(elements) != epsandsig.end()) {
    return epsandsig[elements];
  }
